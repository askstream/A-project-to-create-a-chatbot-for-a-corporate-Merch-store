...
— Приветствие бота и выбор товара из каталога[^1]

— Оформление заказа с указанием телефона и адреса доставки[^2]

— Уведомление администратору о новом заказе[^3]

### 8.2. Административная панель

**Скриншоты веб-интерфейса:**

— **Dashboard**: статистика по товарам, заказам, средний чек, выручка[^4]

— **FAQ**: управление базой знаний с вопросами и ответами[^5]

— **Категории**: список категорий товаров[^6]

— **Детализация категории**: все товары в категории "Одежда" с ценами[^7]

— **Заказы**: список всех заказов с фильтрацией по статусу[^8]

— **Детализация заказа**: информация о клиенте, составе заказа, сумме[^9]
